 ...
 * @author Schmidt
 */
public class TrailingSpaceCorrect extends AbstractConsistencyThing {
    ...
    public static void main(String[] args) {
        AbstractConsistencyThing act = new TrailingSpaceCorrect();
        try {
            act.doit();
            System.out.println(count + " changes");
        } catch (Exception ex) {
            Logger.getLogger(TrailingSpaceCorrect.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void processDocument(Document doc) throws Exception {
        ...
        for (Object o : words){
            ...
            if (wordText.matches(WORD_REGEX) && normText!=null && normText.matches(NORM_REGEX)){
                word.setAttribute("n", word.getAttributeValue("n").trim());
                count++;
            }
        }
        writeDocument();
