...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="ZW_WORD">
        <xsl:element name="w">
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
    
    <xsl:template match="ZW_BOUNDARY">
        <boundary/>
        <xsl:apply-templates select="time"/>
    </xsl:template>
    
    
    <xsl:template match="ZW_INTERRUPT">
        <xsl:element name="interruption"/>
    </xsl:template>
    
    
    <xsl:template match="ZW_PUNCTUATION">
        <xsl:element name="p">
    ...
    <xsl:template match="ZW_SPACE">
        <xsl:apply-templates select="*"/>
    </xsl:template>

   
    <xsl:template match="ZW_COMMENT">
        <comment>
