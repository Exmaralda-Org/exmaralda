...
public class NAMDParser extends AbstractParser {
    ...
    public static NAMDParser createCustomisedGATParser(String languageCode, 
            File patternsFile, 
            File minimalTransformerFile,
            File basicTransformerFile
            ) throws JDOMException, IOException{
        NAMDParser newParser = new NAMDParser(languageCode);
    ...
    public NAMDParser(String languageCode) {
        try {
        ...
        if (parseLevel==1){
            ...
            for (Element unparsed : unparseds){
                ...
                try {
                    ...
                    text = parseText(text, "NAMD_MASK_NAME", minimalPatterns);
                    text = parseText(text, "NAMD_PUNCTUATION", minimalPatterns);
    ...
    void normalizeEXB(BasicTranscription exb) {
        for (int i=0; i<exb.getBody().getNumberOfTiers(); i++){
            Tier tier = exb.getBody().getTierAt(i);
            tier.sort(exb.getBody().getCommonTimeline());
            for (int j=0; j<tier.getNumberOfEvents(); j++){
                tier.getEventAt(j).setDescription(tier.getEventAt(j).getDescription().replaceAll(",", " ,"));
                String text = tier.getEventAt(j).getDescription();
                if (text.startsWith(" ")){
                    tier.getEventAt(j).setDescription(text.substring(1));
                    if (j>0){
                      tier.getEventAt(j-1).setDescription(tier.getEventAt(j-1).getDescription() + " ");
                    }
                }
                tier.getEventAt(j).setDescription(tier.getEventAt(j).getDescription().replaceAll("\\s{2,}", " "));
            }
        }
    }
