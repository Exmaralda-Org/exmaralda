...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="BW_PAUSE">
        <xsl:element name="pause">
            <xsl:attribute name="duration">
                <xsl:choose>
                    <xsl:when test="normalize-space()='+'">unspecified</xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="BW_PUNCTUATION">
        <xsl:element name="p">
