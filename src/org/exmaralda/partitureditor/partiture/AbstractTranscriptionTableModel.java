...
/**
 * implements the basic functionalities for communicating with a JCTable, i.e.
 * implements methods for translating between cells and labels in the table and
 * events etc. in the basic transcription redesigned for version 1.01 for better
 * performance
 *
...
public abstract class AbstractTranscriptionTableModel extends AbstractDataSource implements EditableTableDataModel {
    ...
    /**
     * sets transcription and format table in one go (this can be reasonable for
     * performance reasons
     *
     * @param t
     * @param tft
    ...
    public void setTierFormatTable(TierFormatTable tft) {
        ...
        for (int pos = 0; pos < tierIDs.length; pos++) {     // then set the formats contained in the given format table (ensures that everything has a format
            try {
                formats.setTierFormat(tft.getTierFormatForTier(tierIDs[pos]));
            } catch (JexmaraldaException je) {
            }            // should never get here
        }
    ...
    /**
     * returns the content of the cell viz. the description of the corresponding
     * event
     *
     * @param row
     * @param col
    ...
    /**
     * returns the content of the columns label viz. the description of the
     * corresponding timeline item format is 'n' if no absolute time is assigned
     * and format is 'n [xxx.y]' if an absolute time is assigned
     *
     * @param col
     * @return
     */
    ...
    /**
     * returns the content of the row label viz. the description of speaker and
     * category of the corresponding tier
     *
     * @param row
     * @return
     */
    ...
    public boolean setTableDataItem(final java.lang.Object v, int row, int col) {
        ...
        if (colAdded) {
            fireSelectionChanged(row, getNumColumns() - 1, true);
        }
        try {
    ...
    /**
     * returns the position of the column that holds the timeline item with the
     * specified ID
     */
    ...
    /**
     * returns the position of the row that holds the tier with the specified ID
     */
...
// *********** FIRE METHODS *****************************************
    ...
    /**
     * notifies the listeners that the format of the column labels has been
     * changed
     */
    ...
    /**
     * either returns the event corresponding to the position or throws a
     * JexmaraldaException if there is no such event
     *
     * @param row
     * @param col
     * @return
    ...
    /**
     * returns the text of the speaker contribution that the event in the
     * specified cell is a part of
     *
     * @param row
     * @param col
     * @return
    ...
    /**
     * returns the part of the transcription between the two columns and the
     * specified rows
     *
     * @param rows
     * @param startCol
    ...
    /**
     * returns true if the cell to the right of the specified cell is free,
     * false otherwise if the specified cell is on the right edge of the
     * transcription, also returns true
     */
    ...
    /**
     * returns true if the cell to the left of the specified cell is free, false
     * otherwise if the specified cell is on the left edge of the transcription,
     * also returns true
     */
...
// ******************** METHODS FOR RESIZING ************************************************
    /**
     * returns true if a clear cut at this position is possible i.e. if there
     * are no cells that extend across this column
     */
    ...
    /**
     * returns the nearest column to the left of the specified column at which a
     * clear cut is possible
     *
     * @param col
     * @return
     */
    ...
    /**
     * returns the nearest column to the right of the specified column at which
     * a clear cut is possible
     *
     * @param col
     * @return
     */
    ...
    public int getCellSpan(int row, int col) {
        ...
        try {
            ...
            if (event == null) {
                return 1;
            } // i.e. there is no event before this event
            else if (!timeline.before(tli, event.getEnd())) {
                return 1;
            } else {
                return 0;
            }
