...
public class SrtConverter {
    ...
    public String exportBasicTranscription(BasicTranscription t, boolean basedOnSegmentChains) throws JexmaraldaException {
        //System.out.println(getSRT(t, false, false));
        return getSRT(t, false, false, basedOnSegmentChains);
    }
    ...
    public void writeText(BasicTranscription bt, File file) throws IOException, SAXException, ParserConfigurationException, TransformerException, JexmaraldaException {
        writeText(bt, file, true);
    }
    
    public void writeText(BasicTranscription bt, File file, boolean basedOnSegmentChains) throws IOException, SAXException, ParserConfigurationException, TransformerException, JexmaraldaException {
        String text = exportBasicTranscription(bt, basedOnSegmentChains);
    ...
    public String getSRT(BasicTranscription t, boolean frames, boolean plainText) throws JexmaraldaException {
        return getSRT(t,  frames,  plainText, true);
    }
    
    
    public String getSRT(BasicTranscription t, boolean frames, boolean plainText, boolean basedOnSegmentChains) throws JexmaraldaException {
        StringBuilder sb = new StringBuilder();
        if (basedOnSegmentChains){
            ...
            // #564
            if (!basedOnSegmentChains){
                info = new SegmentedToListInfo(st, SegmentedToListInfo.EVENT_SEGMENTATION);
            }
            ...
            // issue #564: Based on events
            Timeline tl = t.getBody().getCommonTimeline();
            String[] transcriptionTierIDs = t.getBody().getTiersOfType("t");
            int index = 1;
            for (int i=0; i<tl.getNumberOfTimelineItems(); i++){
                TimelineItem tliStart = tl.getTimelineItemAt(i);
                for (String tierID : transcriptionTierIDs){
                    Tier tier = t.getBody().getTierWithID(tierID);
                    if (tier.containsEventAtStartPoint(tliStart.getID())){
                        Event event = tier.getEventAtStartPoint(tliStart.getID());
                        TimelineItem tliEnd = tl.getTimelineItemWithID(event.getEnd());
                        double startTime = tliStart.getTime();
                        double endTime = tliEnd.getTime();
                        String start = TimeStringFormatter.formatSeconds(startTime, true, 3).replace('.', ',');
                        String end = TimeStringFormatter.formatSeconds(endTime, true, 3).replace('.', ',');                        
                        // let's ignore frames, no idea what they are
                        if (!plainText) {
                            sb.append(Integer.toString(index)).append(System.getProperty("line.separator"));
                            sb.append(start).append(" --> ").append(end).append(System.getProperty("line.separator"));
                            ...
                            sb.append(start).append(" ").append(end).append(" ")
                                    .append(event.getDescription()).append(System.getProperty("line.separator"));
                        }
                        index++;
                    }
                }
