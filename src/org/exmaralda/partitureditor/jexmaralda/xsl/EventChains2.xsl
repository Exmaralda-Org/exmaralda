...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
      ...
      <timeline>
        <xsl:for-each select="//tli">
          ...
          <xsl:copy>
            ...
            <xsl:attribute name="count_events_start" select="count(//tier[@type='t']/event[@start=$THIS_ID])"/>
          </xsl:copy>          
        </xsl:for-each>
      </timeline>
    </xsl:variable>
    ...
    <xsl:message select="$SPICED_UP_TIMELINE"/>
    
    <html>
      <head>
        <style>
          tr:nth-child(odd) {background-color: #f2f2f2;}          
          tr:nth-child(even) td {padding-bottom:10px;}          
        </style>
      </head>
      <body>
        <h2><xsl:value-of select="//transcription-name"/></h2>
        <table>
          <xsl:for-each select="//tli">
            <xsl:variable name="THIS_ID" select="@id"/>
            <xsl:variable name="COUNT_START" select="$SPICED_UP_TIMELINE//tli[@id=$THIS_ID]/@count_start"/>
            <xsl:variable name="COUNT_PREVIOUS_START" select="$SPICED_UP_TIMELINE//tli[@id=$THIS_ID]/preceding-sibling::tli[1]/@count_start"/>
            <xsl:variable name="COUNT_EVENTS_START" select="$SPICED_UP_TIMELINE//tli[@id=$THIS_ID]/@count_events_start"/>
            <xsl:variable name="COUNT_PREVIOUS_EVENTS_START" select="$SPICED_UP_TIMELINE//tli[@id=$THIS_ID]/preceding-sibling::tli[1]/@count_events_start"/>
            <xsl:variable name="ID" select="@id"/>
            <xsl:variable name="NEXT_ID" select="following-sibling::tli[1]/@id"/>
            <!-- for all start points -->
            <xsl:if test="$COUNT_START='1' or ($COUNT_EVENTS_START='1' and $COUNT_PREVIOUS_EVENTS_START!='1' and $COUNT_PREVIOUS_START='0')">
              <!-- take care of events starting at that start point -->
              <xsl:choose>
                <xsl:when test="$COUNT_START='1'">
                ...
                <xsl:otherwise>
                  <xsl:apply-templates select="//tier[@type='t']/event[@start=$ID]"/>                  
                </xsl:otherwise>
              </xsl:choose>
  ...
  <xsl:template match="event">
    ...
    <tr>                    
      <xsl:variable name="ABBREVIATION" select="//speaker[@id=$THIS_SPEAKER]/abbreviation"/>
