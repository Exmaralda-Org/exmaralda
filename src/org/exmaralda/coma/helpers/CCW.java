...
public class CCW extends javax.swing.JDialog implements PropertyChangeListener,
    ...
    private HashMap<String, HashMap<String, String>> speakerMetadata = new HashMap<>(); // sprechermetadaten
    ...
    public void tableChanged(TableModelEvent e) {
        ...
        nextButton.setEnabled(!transcriptionTableModel.getSelectedFiles().isEmpty());
    }
    ...
    private void buttonCheck() {
        ...
        switch (cardShowing) {
            ...
            case TRANSCRIPTION_SELECTION:
                ...
                if (!transcriptionTableModel.getSelectedFiles().isEmpty()) {
                    nextButton.setEnabled(true);
                }
                break;
            case SEGMENTATION:
                prevButton.setEnabled(true);
                nextButton.setEnabled(!transcriptions.isEmpty());
                break;
            case METADATA:
                prevButton.setEnabled(true);
                nextButton.setEnabled(true);
            ...
            case SPEAKERS:
                prevButton.setEnabled(true);
                if (!speakerTableModel.getSelectedSpeakers().isEmpty()) {
                    nextButton.setEnabled(true);
                }
                break;
            case SUMMARY:
        ...
        switch (card) {
            case TRANSCRIPTION_SELECTION:
                ...
                for (File f : transcriptions.keySet()) {
                    Vector<Object> innen = new Vector<>();
                    innen.add(true);
                    innen.add(f);
            ...
            case SPEAKERS:
                if (speakerMetaFields == null) {
                    ...
                    for (String s : new TreeSet<>(speakerMetaFields)) {
                        if (!(s.equals("@sex"))) {
    ...
    private void speakerDistinctionChanged(String distinction) {
        ...
        HashSet<String> uniqueSpeakers = new HashSet<>();
        for (File f : transcriptionTableModel.getSelectedFiles()) {
            TranscriptionMetadata transmeta = transcriptions.get(f);
            for (String s : transmeta.getSpeakers().keySet()) {
                // HashMap<String, String> speakerMeta = transmeta.getSpeakers()
                // .get(s);
                uniqueSpeakers.add(transmeta.getSpeakers().get(s).get(distinction));
            }
        }
        Vector<String> ident = new Vector<>();
        ident.add("use");
        ident.add(distinction);
        speakerTableModel.setDataVector(new Vector(), ident);
        if (!uniqueSpeakers.isEmpty()) {
            TreeSet<String> ts = new TreeSet<>();
            try { // sowas macht man möglicherweise nicht, aber jetzt ist's mir
    ...
    private Document getComaDocument() {
        ...
        HashMap<String, Vector<File>> comms = new HashMap<>();
        int count = 0;
        ...
        System.out.println("***** " + fromSegmentation.size() + " fromSegmentation");
        ...
        for (File f : workFiles) {
            ...
            if (commNameCombo.getSelectedItem().equals(
                ...
                String key = transcriptions.get(f).getMetadata().get(selectedCommName);
                String cName = (key == null) ? "unnamed" + count : transcriptions.get(f).getMetadata().get(selectedCommName);
        ...
        HashMap<String, Vector<String>> speakers = new HashMap<>();
        HashMap<String, HashSet<String>> commSpeakers = new HashMap<>();
        HashMap<String, String> speakerNames = new HashMap<>();
        ...
        HashMap<String, Integer> commNames = new HashMap<>();
        ...
        for (String c : comms.keySet()) {
            ...
            if (!commMeta.isEmpty()) {
                ...
                for (String k : new TreeMap<>(actualDescription).keySet()) { // sortiert!
        ...
        for (String sp : speakers.keySet()) {
            HashMap<String, String> myMeta = new HashMap<>(); // Metadaten
                            ...
                            if (!(spM.startsWith("@"))) {
                                ...
                                if (myMetaCount.get(spM) == null) { // diesen schlüssel gibt es noch nicht
                                    ...
                                    for (int i = 0; i < myMetaCount.get(spM); i++) {
                                        if ((myMeta.get(spM + i)
                                                .equals(speakerMetadata.get(spS).get(spM)))) {
                                            difValue = false;
                                        }
                                    }
            ...
            spe.addContent(new Element("Pseudo").setText(myMeta.get(speakerPseudoCombo.getSelectedItem())));
     ...
     *
     * @param c the index of the JLabel in labels to bold
     */
     ...
     *
     * @param c the index of the JLabel in labels to unbold
     */
    ...
    private void selectTranscriptions() {
        ...
        progressMonitor = new ProgressMonitor(this, Ui.getText("ccw.searching"), "", 0, 100);
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if ("progress".equals(evt.getPropertyName())) {
            ...
            String message = progress + "% (" + transcriptions.size() + " found)";
        ...
        private void countFiles(File rootFile) {
            ...
            for (File theFile : theFiles) {
                if (theFile.isDirectory()) {
                    ...
                    if ((theFile.getName().toLowerCase().endsWith(".xml")) || (theFile.getName().toLowerCase().endsWith(".exb") || (theFile.getName().toLowerCase().endsWith(".exs")))) {
                        files.add(theFile);
                    }
                }
            }
    ...
    public void transcriptionsCounted() {
        if (!transcriptions.isEmpty()) {
            numberOfTranscriptionsLabel.setText(transcriptions.size() + " Transcriptions");
            buttonCheck();
        } else {
    ...
    // added 20-02-2023, issue #359
    JLabel customFSMLabel;
    JTextField customFSMTextField;
    JButton customFSMBrowseButton;
    ...
    private void initSegmentationCard() {
        ...
        segSelector.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String segFolderName = (String) ((JComboBox) e.getSource()).getSelectedItem() + "-segmented";
                segTargetOption2TextField.setText(segFolderName);
            }
        });
        main3.add(segSelectLabel);
        main3.add(segSelector);

        // new 20-01-2023: row 3 (issue #359)
        customFSMLabel = new JLabel("Custom FSM: ");
        JPanel customFSMPanel = new JPanel();
        customFSMTextField = new JTextField();
        customFSMTextField.setPreferredSize(new java.awt.Dimension(200, 24));
        customFSMBrowseButton = new JButton("Browse...");
        customFSMBrowseButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                JFileChooser jfc = new JFileChooser();
                jfc.setFileFilter(new ParameterFileFilter("xml", "Finite State Machines (*.xml)"));
                int ret = jfc.showOpenDialog(cardPanel);
        ...
        main3.add(customFSMLabel);
        main3.add(customFSMPanel);

        // row 4: fehlerbehandlung
        ...
        segTargetOption2Panel.add(segTargetOption2TextField, BorderLayout.CENTER);
        ...
        segmentationComponents = new Vector<>();
        ...
        segmentationComponents.add(segTargetOption2TextField);

        segmentationComponents.add(customFSMTextField);
        segmentationComponents.add(customFSMBrowseButton);
        segmentationComponents.add(errorListCheckbox);
    ...
    private void doSegmentation() {
        ...
        if (segTargetOption2.isSelected()) {
            td = new java.io.File(comaFile.getParent(), segTargetOption2TextField.getText());
        }
        ...
        String customFSM = customFSMTextField.getText();
        ...
        final ProgressBarDialog pbd = new ProgressBarDialog((JFrame) (getParent()), false);
        ...
        Thread segmentThread = new Thread() {
            ...
            public void run() {
                try {
                    theSegmentor.doSegmentation(f_sa, f_suff, f_td, f_eh, f_we, f_ep, f_cfsm);
                    System.out.println("------ segmentation done.");
                    segmentedTranscriptions = theSegmentor.getSegmentedTranscriptions();
                ...
                } catch (IOException | JexmaraldaException | SAXException ex) {
