...
public class SpeakerSpecificWordListAction extends org.exmaralda.coma.root.ComaAction {
	...
	public SpeakerSpecificWordListAction(String text, ImageIcon icon, Coma c) {
		super(text, icon, c);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			List transcriptionList = xpath.selectNodes(coma.getData()
					.getDocument());
			...
			if (wordSegmentName == null) {
				JOptionPane.showMessageDialog(coma, "No word segmentation in this corpus.");
				return;
			}
                        
                        // find the currently selected speakers
                        HashMap<String, Element> selectedPersons = coma.getData().getSelectedPersons();
			if (selectedPersons.isEmpty()) {
				JOptionPane.showMessageDialog(coma, "No speakers selected in the speaker list");
				return;
			}
                        
                        
                        Vector<String> selectedSpeakers = new Vector<String>();
                        for (String key : selectedPersons.keySet()){
                            Element speaker = selectedPersons.get(key);
                            selectedSpeakers.add(speaker.getChildText("Sigle"));
                        }
                        final String[] sigles = new String[selectedSpeakers.size()];
                        int count=0;
                        for (String s : selectedSpeakers){
                            sigles[count] = s;
                            System.out.println("Speaker sigle: " + s);
                            count++;
                        }
			...
			final Runnable doDisplayWordList = new Runnable() {
				...
				public void run() {
					try {
						l.readWordsFromExmaraldaCorpus(finalCorpus, sigles);
