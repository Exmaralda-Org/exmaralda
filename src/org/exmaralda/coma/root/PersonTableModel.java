...
public class PersonTableModel extends AbstractTableModel {
	...
	public PersonTableModel() {
		...
		columnNames.add("S");
		columnNames.add("Sigle");
		columnNames.add("Var");
		filter = "";
		try {
			commXPath = XPath.newInstance("Pseudo");
	...
	public void addRow(org.jdom.Element elm) {
		...
		if (add) {
			...
			if (elm.getChild("Sigle").getText().equals("")) {
				...
				data.add(elm.getChild("Sigle").getText());
			}
			data.add(strOut);
	...
	private boolean matchFilter(String compare) {
			 pattern = Pattern.compile(filter);
		     matcher = pattern.matcher(compare);
		     return matcher.matches();
/*		String compFilter = filter;
		boolean posNeg = true;
		if (filter.startsWith("+")) {
			compFilter = filter.substring(1);
		} else if (filter.startsWith("-")) {
			compFilter = filter.substring(1);
			posNeg = false;
		}
		if (compFilter.equals(compare)) {
			return posNeg;
		} else {
			return !posNeg;
		}*/
	}
	public boolean setSecondColumnFilter(String newFilter) {
		filter = newFilter;
		return true;
	}
	/**
	 ...
	 * @return
	 */
	public int getRowForElement(Element elm) {
		return data.indexOf(elm) / 4;
	}
}