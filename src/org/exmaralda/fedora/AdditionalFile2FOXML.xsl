...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:foxml="info:fedora/fedora-system:def/foxml#">
    ...
    <!-- Parameter passed to the stylesheet, e.g. 'hamatac' -->
    ...
    <xsl:template match="/">
        <foxml:digitalObject VERSION="1.1" xmlns:foxml="info:fedora/fedora-system:def/foxml#"
            ...
            <xsl:attribute name="PID">add:<xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="//Communication/@Name"/></xsl:attribute>
            
            <!--  <File Id="FIDBC34AB3B-9527-6D57-F204-466FD3C5E8EB">
                <filename>Dimitri.BMP</filename>
                <mimetype>unknown</mimetype>
                <relPath>Ali_Dimitri/Dimitri.BMP</relPath>
                <absPath>T:\TP-Z2\DATEN\MAPTASK\Map_Task_Aufnahmen\Ali_Dimitri\Dimitri.BMP</absPath>
                <Description />
                </File>
            -->
            <!-- *****************************************************  -->
            <foxml:objectProperties>
                ...
                <foxml:property NAME="info:fedora/fedora-system:def/model#label">
                    <xsl:attribute name="VALUE">Additional file <xsl:value-of select="//File[1]/filename"/></xsl:attribute>
                </foxml:property>
            ...
            <xsl:call-template name="ADDITIONAL_FILE_STREAM"/>
        ...
        <foxml:datastream CONTROL_GROUP="M" ID="COLLECTION_VIEW" STATE="A" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <foxml:contentLocation>
                    <xsl:attribute name="REF">http://hrsk.multilingua.uni-hamburg.de/xsl/collection_view_add.xsl</xsl:attribute>
    ...
    <xsl:template name="ADDITIONAL_FILE_STREAM">
    
        <!-- the wav recording -->
        <foxml:datastream CONTROL_GROUP="M" ID="BMP" STATE="A" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <xsl:attribute name="CREATED"><xsl:value-of select="current-dateTime()"/></xsl:attribute>
                <xsl:attribute name="ID">bmp.0</xsl:attribute>
                <xsl:attribute name="LABEL"><xsl:value-of select="//File[1]/filename"/></xsl:attribute>
                <xsl:attribute name="MIMETYPE">image/bmp</xsl:attribute>
                ...
                <foxml:contentLocation>
                    ...
                    <xsl:attribute name="REF"><xsl:value-of select="//File[ends-with(relPath, '.BMP')]/relPath"/></xsl:attribute>
    ...
    <xsl:template name="TN_STREAM">    
        ...
        <foxml:datastream CONTROL_GROUP="M" ID="TN" STATE="A" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <xsl:attribute name="LABEL">add-icon</xsl:attribute>
                <xsl:attribute name="MIMETYPE">image/png</xsl:attribute>
                <foxml:contentLocation>
                    <xsl:attribute name="REF">http://localhost:8080/fedora/get/hzsk:add-icon/tn</xsl:attribute>
                    <xsl:attribute name="TYPE">URL</xsl:attribute>
                </foxml:contentLocation>
            </foxml:datastreamVersion>
        </foxml:datastream>        
            ...
            <foxml:datastreamVersion CREATED="2011-10-07T13:54:12.093Z" FORMAT_URI="http://www.openarchives.org/OAI/2.0/oai_dc/" ID="DC1.0" LABEL="Dublin Core Record for this object" MIMETYPE="text/xml" SIZE="450">
                <foxml:xmlContent>
                    <oai_dc:dc 
                        ...
                        <dc:title>Additional file <xsl:value-of select="//File[1]/filename"/></dc:title>
                        <dc:identifier>
                            <xsl:text>add:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="//Communication/@Name"/>                            
                        </dc:identifier>
                    </oai_dc:dc>
                </foxml:xmlContent>
            </foxml:datastreamVersion>
                    ...
                    <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                        <rdf:Description>
                            ...
                            <xsl:attribute name="rdf:about">                                
                                <xsl:text>info:fedora/add:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="//Communication/@Name"/>
                            </xsl:attribute>                            
                            <!-- information about the external relation 'hasModel' (= ContentModel for the transcription objects) -->
                            <hasModel rdf:resource="info:fedora/add:cmodel" xmlns="info:fedora/fedora-system:def/model#"/>
                            ...
                            <isMemberOf xmlns="info:fedora/fedora-system:def/relations-external#">
                                <xsl:attribute name="rdf:resource"><xsl:text>info:fedora/col:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_add</xsl:attribute>                                                            
                            </isMemberOf>
                        </rdf:Description>
                    </rdf:RDF>
                </foxml:xmlContent>
