...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:foxml="info:fedora/fedora-system:def/foxml#">
    ...
    <xsl:template match="/">
        <foxml:digitalObject VERSION="1.1" xmlns:foxml="info:fedora/fedora-system:def/foxml#"
            ...
            <xsl:attribute name="PID">spk:<xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="translate(//Sigle,'ÄÖÜäöüßé','AOUaouse')"/></xsl:attribute>
            
            <!-- *****************************************************  -->
            <foxml:objectProperties>
                <foxml:property NAME="info:fedora/fedora-system:def/model#state" VALUE="Active"/>
                <foxml:property NAME="info:fedora/fedora-system:def/model#label">
                    <xsl:attribute name="VALUE">Speaker <xsl:value-of select="//Sigle"/><xsl:if test="string-length(//Pseudo)&gt;0"> (<xsl:value-of select="//Pseudo"/>)</xsl:if></xsl:attribute>
                </foxml:property>
        ...
        <foxml:datastream CONTROL_GROUP="M" ID="COLLECTION_VIEW" STATE="A" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <foxml:contentLocation>
                    <xsl:attribute name="REF">http://hrsk.multilingua.uni-hamburg.de/xsl/collection_view_spk.xsl</xsl:attribute>
    ...
    <xsl:template name="COLLECTION_POLICY_STREAM">
        <foxml:datastream ID="COLLECTION_POLICY" STATE="A" CONTROL_GROUP="E" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <xsl:attribute name="MIMETYPE">text/xml</xsl:attribute>
                <foxml:contentLocation
                    REF="http://localhost:8080/fedora/get/hzsk:spk_CollectionPolicy/COLLECTION_POLICY"
                    TYPE="URL"/>                
            </foxml:datastreamVersion>
        </foxml:datastream>        
    </xsl:template>    
    ...
    <xsl:template name="TN_STREAM">    
        ...
        <foxml:datastream CONTROL_GROUP="M" ID="TN" STATE="A" VERSIONABLE="true">
            <foxml:datastreamVersion>
                ...
                <xsl:attribute name="LABEL">speaker-icon</xsl:attribute>
                <xsl:attribute name="MIMETYPE">image/png</xsl:attribute>
                <foxml:contentLocation>
                    <xsl:attribute name="REF">http://localhost:8080/fedora/get/hzsk:col-spk-icon/tn</xsl:attribute>
                    <xsl:attribute name="TYPE">URL</xsl:attribute>
                </foxml:contentLocation>
            </foxml:datastreamVersion>
        </foxml:datastream>                
            ...
            <foxml:datastreamVersion CREATED="2011-10-07T13:54:12.093Z" FORMAT_URI="http://www.openarchives.org/OAI/2.0/oai_dc/" ID="DC1.0" LABEL="Dublin Core Record for this object" MIMETYPE="text/xml" SIZE="450">
                <foxml:xmlContent>
                    <oai_dc:dc 
                        ...
                        <dc:title>Speaker <xsl:value-of select="//Sigle"/><xsl:if test="string-length(//Pseudo)&gt;0"> (<xsl:value-of select="//Pseudo"/>)</xsl:if></dc:title>
                        <dc:identifier>
                            <xsl:text>spk:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="translate(//Sigle,'ÄÖÜäöüßé','AOUaouse')"/>                            
                        </dc:identifier>
                    </oai_dc:dc>
                </foxml:xmlContent>
            </foxml:datastreamVersion>
            ...
            <foxml:datastreamVersion CREATED="2011-10-07T13:54:12.093Z" FORMAT_URI="http://www.clarin.eu/cmd" ID="CMDI1.1" LABEL="CMDI metadata for this object" MIMETYPE="text/xml">
                <foxml:xmlContent>
                    <CMD CMDVersion="1.1" 
                        ...
                        <Actor ComponentId="clarin.eu:cr1:c_1302702320452">
                            <Pseudonym><xsl:value-of select="//Sigle"/></Pseudonym>
                        </Actor>                               
                    </CMD>
                </foxml:xmlContent>
            </foxml:datastreamVersion>
                    ...
                    <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                        <rdf:Description>
                            ...
                            <xsl:attribute name="rdf:about">                                
                                <xsl:text>info:fedora/spk:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="translate(//Sigle,'ÄÖÜäöüßé','AOUaouse')"/>
                            </xsl:attribute>                            
                            <!-- information about the external relation 'hasModel' (= ContentModel for the communication objects) -->
                            <hasModel rdf:resource="info:fedora/spk:cmodel" xmlns="info:fedora/fedora-system:def/model#"/>
                            ...
                            <!--  am 5. Juli 2012 in geänderter Form wieder rinngemacht-->
                            <isMemberOf xmlns="info:fedora/fedora-system:def/relations-external#">
                                <xsl:attribute name="rdf:resource"><xsl:text>info:fedora/col:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_spk</xsl:attribute>                                                            
                            </isMemberOf>
                            
                            <xsl:for-each select="descendant::Communication">
                                <isMemberOf xmlns="info:fedora/fedora-system:def/relations-external#">
                                    <xsl:attribute name="rdf:resource">                                
                                        <xsl:text>info:fedora/com:</xsl:text><xsl:value-of select="$CORPUS_NAMESPACE"/>_<xsl:value-of select="@Name"/>
                                    </xsl:attribute>                            
                                </isMemberOf>
                            </xsl:for-each>
