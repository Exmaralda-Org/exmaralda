...
import org.exmaralda.common.helpers.*;
import org.xml.sax.*;
import java.util.*;
...
public class FOLKERInternationalizer {
    ...
    static {
        try{
            ...
            java.io.InputStream is = FOLKERInternationalizer.class.getResourceAsStream(INTERNATIONALIZATION_FILENAME);
            xmlReader.parse(new org.xml.sax.InputSource(is));     



            String language = java.util.prefs.Preferences.userRoot().
                                node("org.exmaralda.folker").
                                get("language","xx");

            if (language.equals("xx")){
                // get the language according to the locale
                String locale = Locale.getDefault().getISO3Language();
                System.out.println("Locale: " + locale);
                if (locale.equals("deu")){
                    language="de";
    ...
    public static void internationalizeDialogToolTips(javax.swing.JDialog dialog){
        dialog.setTitle(FOLKERInternationalizer.getString(dialog.getTitle()));        
    ...
    public static void internationalizeComponentToolTips(java.awt.Component comp){
        ...
        if (comp instanceof javax.swing.JTabbedPane){
            ...
            for (int pos=0; pos<tabbedPane.getTabCount(); pos++){
                tabbedPane.setToolTipTextAt(pos, FOLKERInternationalizer.getString(tabbedPane.getTitleAt(pos)));
                if (internationalizeAll){
                    tabbedPane.setTitleAt(pos, FOLKERInternationalizer.getString(tabbedPane.getTitleAt(pos)));
                }
            }
        }
