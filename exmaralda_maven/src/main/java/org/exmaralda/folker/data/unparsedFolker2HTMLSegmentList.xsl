...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exmaralda="http://www.exmaralda.org"  version="2.0">
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="SORTED-SEGMENTS">
            <xsl:element name="sorted-segments">
                <xsl:copy-of select="//timeline"/>
                <xsl:for-each select="//segment">
                    <xsl:sort select="//timepoint[@timepoint-id=current()/@start-reference]/@absolute-time" data-type="number"/>
                    <xsl:sort select="//timepoint[@timepoint-id=current()/@end-reference]/@absolute-time" data-type="number" order="descending"/>
                    <xsl:copy>
                        <xsl:attribute name="speaker-reference">
                            <xsl:value-of select="ancestor::*[1]/@speaker-reference"/>
                        </xsl:attribute>
                        <xsl:copy-of select="@*"/>
        ...
        <html>
            ...
            <body>
                <table>
                    <xsl:apply-templates select="$SORTED-SEGMENTS/descendant::segment"/>
                </table>
            </body>
        </html>
    </xsl:template>
    ...
    <xsl:template match="segment">
        <tr>
            ...
            <td class="numbering">
                <xsl:variable name="NUMBER"><xsl:value-of select="number($COUNT_START) + number(position())"/></xsl:variable>
                <xsl:if test="($NUMBER + 0) &lt; 10"><xsl:text>0</xsl:text></xsl:if>
                <xsl:if test="($NUMBER + 0) &lt; 100"><xsl:text>0</xsl:text></xsl:if>
                <xsl:if test="($NUMBER + 0) &lt; 1000"><xsl:text>0</xsl:text></xsl:if>
                <xsl:value-of select="$NUMBER"/>                
            </td>
            <td class="speaker">
                <xsl:if test="not(preceding-sibling::*[1]/@speaker-reference=current()/@speaker-reference)">
