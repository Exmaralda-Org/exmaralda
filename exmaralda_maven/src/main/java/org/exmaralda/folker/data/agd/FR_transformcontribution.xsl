...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="FR_BOUNDARY">
        <boundary/>
        <xsl:apply-templates select="time"/>
    </xsl:template>

    
    <xsl:template match="FR_PUNCTUATION">
        <xsl:element name="p">
    ...
    <xsl:template match="FR_SPACE">
        <xsl:apply-templates select="*"/>
    </xsl:template>
    
    <xsl:template match="FR_INTONATION|FR_ONE_PART_ANNOTATION|FR_TWO_PART_ANNOTATION">
    ...
    <xsl:template match="FR_EMPHASIS">
        <xsl:element name="annotation">
            <xsl:attribute name="category">emphasis</xsl:attribute>            
        </xsl:element>
    </xsl:template>
